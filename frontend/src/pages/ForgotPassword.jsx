...
import { Shield, Mail, Lock, KeyRound, AlertCircle, CheckCircle2, ArrowRight, RefreshCw } from 'lucide-react';
...
const ForgotPassword = () => {
  ...
  const handleRequest = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Error requesting reset.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center px-4 sm:px-6 lg:px-8 relative overflow-hidden selection:bg-emerald-500 selection:text-slate-950">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center z-10">
        ...
        <p className="mt-1.5 text-xs text-slate-400">
          Restore secure access to your BharatSHIELD console
        </p>
      </div>
        ...
        <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-800/90 py-8 px-6 sm:px-8 rounded-3xl shadow-2xl shadow-slate-950/80 space-y-6">
          ...
          {step === 1 && (
            <form onSubmit={handleRequest} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-sm font-bold shadow-lg shadow-emerald-950/40 flex items-center justify-center gap-2 transition active:scale-[0.98] disabled:opacity-50"
              >
                {isLoading ? <RefreshCw size={16} className="animate-spin" /> : <span>Send Recovery Token</span>}
              </button>
            </form>
          )}
          ...
          {step === 2 && (
            <form onSubmit={handleReset} className="space-y-4">
              {devToken && (
                <div className="p-3 bg-slate-950 border border-emerald-500/30 rounded-xl text-xs space-y-1">
                  <div className="font-mono text-[10px] text-emerald-400 font-bold uppercase">
                    Development Mode Recovery Link:
                  </div>
                  <div className="font-mono text-slate-300 break-all">{devToken}</div>
                </div>
              )}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="rst_..."
                    className="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 font-mono transition"
                  />
                  <KeyRound size={16} className="absolute right-3.5 top-3 text-slate-500" />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-sm font-bold shadow-lg shadow-emerald-950/40 flex items-center justify-center gap-2 transition active:scale-[0.98] disabled:opacity-50"
              >
                {isLoading ? <RefreshCw size={16} className="animate-spin" /> : <span>Update Password</span>}
              </button>
            </form>
          )}
          ...
          {step === 3 && (
            <div className="text-center py-4 space-y-4">
              ...
              <Link
                to="/login"
                className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-emerald-500 text-slate-950 font-bold text-sm hover:bg-emerald-400 transition"
              >
